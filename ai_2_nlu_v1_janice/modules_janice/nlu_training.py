...
def train_transformer(
    ...
    # Hitung total training steps dulu
    total_steps = (len(train_dataset) // 16) * epochs   # 16 = per_device_train_batch_size yang kamu pakai
    warmup_steps = int(0.1 * total_steps)   # 0.1 = rasio warmup yang tadinya kamu pakai
    ...
    training_args = TrainingArguments(
        ...
        weight_decay=0.01,
        warmup_steps=warmup_steps,     # ganti dari warmup_ratio
...
HOSTAGE_BENCHMARK_TESTS = (
    ...
    ("defend", "Aku Spy dan semalam Guard Raka, jadi jangan curigai dia."),       # dulu: bluffing
    ("neutral", "Stalker, kamu Peek siapa semalam dan apa hasil yang kamu lihat?"),
    ("offend", "Jangan hanya fokus ke aku; cek D yang ceritanya berubah setiap ditanya."),  # dulu: deflecting
    ("offend", "Vote C saja, dia paling diuntungkan ketika seseorang mendadak diam."),       # dulu: persuading
    ("defend", "Klaimku Civilian, jadi aku memang tidak memiliki aksi malam."),   # dulu: claiming
...
def run_all_nlu_models(
    ...
    *,
    ...
    print("DEBUG jumlah artifacts:", len(artifacts))
    print("DEBUG artifacts keys:", list(artifacts.keys()))
    ...
    if not manual_rows:
        raise RuntimeError("artifacts kosong — tidak ada model yang berhasil ditraining, cek langkah training CPU model di atas.")
...
def predict_intent_full_probabilities(text: str, model_dir, filename=None) -> dict[str, float]:
    model_path = resolve_model_path(model_dir, filename)
    model = joblib.load(model_path)
    probs = model.predict_proba([str(text)])[0]
    return {str(label): round(float(p) * 100, 2) for label, p in zip(model.classes_, probs)}